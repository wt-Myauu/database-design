...
CREATE TABLE users
(
    user_id    BIGINT       NOT NULL AUTO_INCREMENT COMMENT '유저 대리키',
  ...
  COLLATE = utf8mb4_0900_ai_ci COMMENT = '서비스 이용자. 행동 주체 구분이 목적';
```
...
| POST | `/api/auth/login` | X | 로그인, 액세스 토큰 발급 |
| GET | `/api/auth/check-login-id?loginId=` | X | 아이디 중복 확인 |
| GET | `/api/users/me` | O | 내 정보 조회 (토큰 동작 확인용) |

인증은 **JWT(HS256) 액세스 토큰 단일 방식**이다. 무상태라 세션 테이블이 필요 없고,
DB 설계를 유저 스키마 자체에 집중시킬 수 있다. 리프레시 토큰은 범위 밖이라 두지 않았다.
